...
namespace CustomizeIt.AI.Office
{
    public partial class CustomizableOfficeBuildingAI : OfficeBuildingAI, ICustomAI
    {
        ...
        private void AssignClass()
        {
            ...
            itemClass.Append("Office - ");
            if (m_info.m_mesh.name.ToLower().Contains("hightech") && SteamHelper.IsDLCOwned(SteamHelper.DLC.GreenCitiesDLC))
                itemClass.Append("Hightech");
            else
            {
            ...
            try
            {
                switch (item.m_class.m_subService)
                {
                    case ItemClass.SubService.OfficeHightech:
                        array = RPCData.officeHighTech;
                        break;

                    case ItemClass.SubService.OfficeGeneric:
                    default:
                        array = RPCData.office;
                        break;
                }
